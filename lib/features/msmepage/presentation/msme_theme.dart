...
abstract final class MsmeTheme {
	...
	static TextStyle headingLarge({Color? color}) => GoogleFonts.plusJakartaSans(fontSize: 24, fontWeight: FontWeight.w800, color: color ?? textWhite);
	static TextStyle headingMedium({Color? color}) => GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w700, color: color ?? textWhite);
	static TextStyle headingSmall({Color? color}) => GoogleFonts.plusJakartaSans(fontSize: 15, fontWeight: FontWeight.w700, color: color ?? textWhite);
	static TextStyle body({Color? color, double size = 14, FontWeight weight = FontWeight.w400}) => GoogleFonts.inter(fontSize: size, fontWeight: weight, color: color ?? textWhite);
	static TextStyle label({Color? color, double size = 11}) => GoogleFonts.inter(fontSize: size, fontWeight: FontWeight.w600, color: color ?? textMuted, letterSpacing: 0.5);
}

class MsmeBadge extends StatelessWidget {
	const MsmeBadge({super.key, required this.label, this.type = MsmeBadgeType.orange, this.icon});
		...
		return Container(
			...
			child: Row(
				...
				children: [
					...
					Text(label.toUpperCase(), style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w700, color: fg, letterSpacing: 0.5)),
				],
			),
		);
	}

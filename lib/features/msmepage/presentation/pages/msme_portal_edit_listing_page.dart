...
class MsmePortalEditListingPage extends ConsumerStatefulWidget {
	const MsmePortalEditListingPage({super.key, required this.listingId});

	final String listingId;

	@override
	ConsumerState<MsmePortalEditListingPage> createState() => _MsmePortalEditListingPageState();
}

class _MsmePortalEditListingPageState extends ConsumerState<MsmePortalEditListingPage> {
	...
	Widget build(BuildContext context) {
		...
		listingsAsync.whenData((listings) {
			if (!_initialized) {
				final match = listings.firstWhere((l) => l['id'].toString() == widget.listingId, orElse: () => {});
				if (match.isNotEmpty) {
					_nameCtrl.text = match['name'] ?? '';
					_priceCtrl.text = (match['price'] ?? 0).toString();
					_capacityCtrl.text = (match['capacity'] ?? 1).toString();
					_hoursCtrl.text = match['hours'] ?? '';
					_descCtrl.text = match['description'] ?? '';
					_imageUrlCtrl.text = match['image'] ?? '';
					_selectedCategory = match['category'] ?? 'Handicrafts';
					_isFeatured = match['is_featured'] == true;
				}
				_initialized = true;
			}
		});
						...
						Row(
							children: [
								...
								Column(
									crossAxisAlignment: CrossAxisAlignment.start,
									children: [
										Text('Edit Business Listing', style: MsmeTheme.headingLarge()),
										Text('Modify listing details, availability, and pricing.', style: MsmeTheme.body(color: MsmeTheme.textMuted)),
									],
								),
							],
						),
									...
									children: [
										...
										Row(
											children: [
												...
												const SizedBox(width: AppSpacing.md),
												Expanded(child: _buildInputField('Cover Image URL', _imageUrlCtrl, Icons.image_rounded)),
											],
										),
										const SizedBox(height: AppSpacing.md),
										...
										Row(
											...
											children: [
												...
												ElevatedButton.icon(
													...
													label: Text(_saving ? 'Saving...' : 'Update Listing', style: GoogleFonts.inter(fontWeight: FontWeight.bold)),
												),
											],
										),
									],
